...
function mealPlanRef(uid) {
  return doc(db, 'meal_plans', uid);
}
...
async function fetchWeeklyMeals() {
  ...
  const snap = await getDoc(mealPlanRef(user.uid));
  if (snap.exists()) {
    weeklyMeals = { ...weeklyMeals, ...(snap.data().weekly_meals || {}) };
    updateAllDayCards();
  }
}
...
async function saveWeeklyMeals() {
  ...
  try {
    await setDoc(mealPlanRef(user.uid), { weekly_meals: weeklyMeals }, { merge: true });
  } catch (error) {
...
function openCustomizer(day) {
  ...
  currentMeal = weeklyMeals[day] ? { ...weeklyMeals[day] } : { protein: null, side1: null, side2: null };
}
...
function updateAllDayCards() {
  Object.keys(weeklyMeals).forEach((day) => updateDayCard(day));
  updateProgress();
}
